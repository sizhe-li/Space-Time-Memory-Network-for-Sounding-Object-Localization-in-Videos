...
class VGG(nn.Module):
    def __init__(self, original_vgg16):
        ...
        layers = list(original_vgg16.features.children())[:-7]
        layers[0] = nn.Conv2d(1,  64, kernel_size=3, padding=1, stride=2)
...
class VGGish(nn.Module):
    ...
    def forward(self, x):
        """
            Args: 
                x (torch.tensor) : spectrogram of shape [B, C, H, W]
            Returns:
                x (torch.tensor) : features of shape [B, C', H', W']
        """
